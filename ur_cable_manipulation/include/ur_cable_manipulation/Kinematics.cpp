...
// using namespace ur_kinematics;
  ...
  Kinematics::Kinematics(const std::string& URDF_name) {

    if (strcmp(URDF_name.c_str(), "ur5") == 0) {
      d1 =  0.089159;
      a2 = -0.42500;
      a3 = -0.39225;
      d4 =  0.10915;
      d5 =  0.09465;
      d6 =  0.0823;
    } else if (strcmp(URDF_name.c_str(), "ur5e") == 0) {
      d1 =  0.1625;
      a2 = -0.425;
      a3 = -0.3922;
      d4 =  0.1333;
      ...
      ROS_ERROR("Kinematics::Kinematics(): Unknown URDF name '%s'", URDF_name.c_str());
    }
  }

  Kinematics::Kinematics(Kinematics::ROBOT_TYPE robot_type){
    ...
    switch (robot_type){
      ...
      case Kinematics::UR3:
        ...
        break;
      default:
        ROS_ERROR("Kinematics::Kinematics(): Unknown robot type");
        break;
    }
  }

  std::vector<double> Kinematics::tfPose_quat(std::string ee_Frame, std::string bl_Frame){

    tf::StampedTransform transform;
    try{
      listener.lookupTransform(bl_Frame, ee_Frame,
    ...
    pose.push_back(transform.getOrigin().x());
    pose.push_back(transform.getOrigin().y());
    pose.push_back(transform.getOrigin().z());
    pose.push_back(transform.getRotation().x());
    pose.push_back(transform.getRotation().y());
    pose.push_back(transform.getRotation().z());
    pose.push_back(transform.getRotation().w());

    return pose;

  }

  std::vector<double> Kinematics::tfPose_rpy(std::string ee_Frame, std::string bl_Frame){
  ...
  Eigen::Matrix4d Kinematics::Vector_To_HomogeneusMatrix(double x, double y, double z, double roll, double pitch, double yaw)
  {    
      Eigen::Matrix4d T;
