...
public:
    DualPlanning() :
        isFirst_mass_grasped(false),
        isLast_mass_grasped(false),
        bool_pub_(nh_.advertise<std_msgs::Bool>("grasp_info_topic", 10)),
        // service(nh_.advertiseService("grasp_info_service", &DualPlanning::serviceCallback, this))
        grasp_client(nh_.serviceClient<cable_model::CableMsg>("set_cable_grasp"))
    {
        // Start a new thread to publish the bool message
        publish_thread_ = std::thread(&DualPlanning::publishLoop, this);
    ...
    void run()
    {
        ...
        current_ur5e_arm_pose = ur5e_move_group_interface.getCurrentPose();
        target_ur5e_arm_pose = current_ur5e_arm_pose;
        target_ur5e_arm_pose.pose.position.x -= 0.95;
        ur5e_waypoints.push_back(target_ur5e_arm_pose.pose);
...
private:
    void publishLoop()
    {
        ...
        while (ros::ok())
        {
            // Publish the current bool value
            std_msgs::Bool bool_msg;
            bool_msg.data = isFirst_mass_grasped;
            bool_pub_.publish(bool_msg);
            rate.sleep();
        }
    }

    void serviceCallback(const std_msgs::Bool::ConstPtr& msg)
    {
        ROS_INFO("Received service request");
        isFirst_mass_grasped = msg->data;
    }

    bool isFirst_mass_grasped;
    bool isLast_mass_grasped;
    ros::NodeHandle nh_;
...
{
    ros::init(argc, argv, "grasper_node");
    DualPlanning node;
    node.run();
